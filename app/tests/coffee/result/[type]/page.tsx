...
export function generateStaticParams(): Params[] {
  return Object.keys(coffeeTest.results).map((type) => ({ type }));
}
...
export async function generateMetadata({
  ...
  const result = coffeeTest.results[type];
  if (!result) return { title: "결과를 찾을 수 없어요" };
  const title = `${result.title} · ${coffeeTest.title}`;
  const url = `/test/tests/${coffeeTest.id}/result/${type}`;
  return {
    title,
...
export default async function CoffeeResultPage({
  params,
}: {
  params: Promise<Params>;
}) {
  ...
  const result = coffeeTest.results[type];
  if (!result) notFound();
  const testUrl = `${siteUrl}/test/tests/${coffeeTest.id}`;
  const resultUrl = `${testUrl}/result/${type}`;
  return (
    <>
      <JsonLd
        data={[
          resultJsonLd(coffeeTest, result),
          breadcrumbJsonLd([
            { name: siteName, url: siteUrl },
            { name: coffeeTest.title, url: testUrl },
            { name: result.title, url: resultUrl },
          ]),
        ]}
      />
      <ResultCard
        testId={coffeeTest.id}
        testTitle={coffeeTest.title}
